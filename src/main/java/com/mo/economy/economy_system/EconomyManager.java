...
public class EconomyManager extends PersistentState {
    ...
    private final Map<UUID, RedPackManager> welfares = new HashMap<>();
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        ...
        for (Map.Entry<UUID, RedPackManager> entry : welfares.entrySet()) {
    ...
    public static EconomyManager fromNbt(NbtCompound nbt) {
        ...
        for (String key : welfaresNbt.getKeys()) {
            manager.welfares.put(UUID.fromString(key), RedPackManager.fromNbt(welfaresNbt.getCompound(key)));
        }
    ...
    public UUID createWelfare(ServerPlayerEntity player, double totalAmount, int count, boolean isLucky) {
        ...
        RedPackManager redPack = new RedPackManager(welfareId, player.getUuid(), totalAmount, count, isLucky);
    ...
    public double claimWelfare(UUID welfareId, ServerPlayerEntity player) {
        RedPackManager redPack = welfares.get(welfareId);
